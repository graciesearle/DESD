...
def _cart_summary(cart):
    ...
    """
    items = cart.items.select_related(
        "product", "product__producer", "product__farm", "product__category"
        ...
        cart_items_by_producer.setdefault(producer_name, []).append(
            {
                ...
                else "https://placehold.co/120x120?text=No+Image",
    ...
    return JsonResponse(
        {
            ...
            "grand_total": str(summary["grand_total"]),
            "producer_subtotals": {
                k: str(v) for k, v in summary["producer_subtotals"].items()
            },
        }
    )

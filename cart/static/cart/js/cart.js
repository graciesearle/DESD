...
(function () {
  ...
  function cleanEmptyGroups() {
    document
      .querySelectorAll("[data-producer-group]")
  ...
  document.addEventListener("click", function (e) {
    ...
    if (action === "decrease-qty") {
      ...
      cartFetch("/cart/api/update/" + itemId + "/", "PATCH", {
        quantity: newQty,
      }).then(function (res) {
        if (!res.ok) {
          showToast(res.data.error || "Error", 3000);
          return;
        }
        qtyInput.value = res.data.quantity;
        row.querySelector('[data-display="item-total"]').textContent =
          "£" + res.data.item_total;
    ...
    // --- Remove ---
      ...
      cartFetch("/cart/api/remove/" + itemId + "/", "DELETE").then(
        function (res) {
          if (!res.ok) {
            showToast(res.data.error || "Error", 3000);
            return;

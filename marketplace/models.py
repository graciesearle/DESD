...
class Category(models.Model):
    ...
    slug = models.SlugField(
        unique=True,
        blank=True,
        help_text="Used to filter in the URL (Automatically filled)",
    )
    ...
    )  # Allow 'Uncategorised' to be created without image.

    class Meta:
        verbose_name_plural = (
            "Categories"  # As this would automatically pluralise as Categorys
        )
    ...
    def save(self, *args, **kwargs):
        if not self.slug:  # Don't overwrite an existing slug.
            self.slug = slugify(
                self.name
            )  # Converts it to a slug (lowercase, spaces handled, etc...).
        super().save(
            *args, **kwargs
        )  # Calls djangos original save method to save to database.

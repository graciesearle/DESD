    ...
    def clean(self):  # Post validation
        ...
        if name and postcode and self.user:
            queryset = Farm.objects.filter(
                producer=self.user, name__iexact=name, postcode__iexact=postcode
            )
            ...
            if queryset.exists():
                self.add_error(
                    "name",
                    "You already have another farm registered with this exact name and postcode.",
                )
...
class ProductAddForm(forms.ModelForm):
    ...
    unit = forms.ChoiceField(
        choices=UNIT_CHOICES, widget=forms.Select(attrs={"class": "form-control"})
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user:
            ...
            if not user_farms.exists():
                self.fields[
                    "farm"
                ].empty_label = "No farm registered - Please register a farm first."
    ...
    def clean(self):
        ...
        if name and farm and self.user:
            ...
            queryset = Product.objects.filter(
                producer=self.user, name__iexact=name, farm=farm
            )
            if self.instance and self.instance.pk:
                queryset = queryset.exclude(pk=self.instance.pk)
            ...
            if queryset.exists():
                self.add_error(
                    "name",
                    f"You already have a product named '{name}' registered for '{farm.name}'.",
                )
        ...
        if is_available and stock is not None and stock == 0:
            self.add_error(
                "is_available",
                "You cannot mark a product as 'Available' if the stock quantity is 0. Please uncheck this box or add stock.",
            )
        ...
        if not is_year_round:
            ...
            if not season_start and not season_end:
                self.add_error(
                    "season_start",
                    "Please specify at least a start or end date, or check 'Available All Year-Round?'.",
                )

            # If they provide both:
            elif season_start and season_end and season_start > season_end:
                self.add_error(
                    "season_end", "Season end date cannot be before the start date."
                )

        if harvest_date:
            if harvest_date > date.today():
                self.add_error(
                    "harvest_date", "Harvest date cannot be set in the future."
                )

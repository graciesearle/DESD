...
def admin_required(
    view_func=None, *, redirect_url=None, message="Administrator access is required."
):
...
def community_group_required(
    ...
    *,
    ...
    decorator = _role_required(
        "COMMUNITY_GROUP", redirect_url=redirect_url, message=message
    )
    if view_func is not None:
        return decorator(view_func)
    return decorator
...
def producer_or_admin_required(
    view_func=None,
    *,
    redirect_url=None,
    ...
    decorator = _role_required(
        "PRODUCER", "ADMIN", redirect_url=redirect_url, message=message
    )
    if view_func is not None:
        return decorator(view_func)
    return decorator

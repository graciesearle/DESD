...
class ProductManager(SoftDeleteManager):
    def active_and_in_season(self):
        ...
        return (
            self.select_related("category", "producer", "farm")
            .prefetch_related("allergens")
            .filter(  # fetch their category, producer and farm while you are fetching products
...
class Farm(SoftDeleteModel):
    ...
    description = models.TextField(
        blank=True, help_text="Tell the community about your farm."
    )
...
class Product(SoftDeleteModel):
    ...
    is_year_round = models.BooleanField(
        default=False,
        verbose_name="Available all year round?",
        help_text="If checked, seasonal start and end dates will be ignored.",
    )
    season_start = models.DateField(
        null=True, blank=True, help_text="When does the season start?"
    )
    season_end = models.DateField(
        null=True, blank=True, help_text="When does the season end?"
    )

    # TC-004: Harvest Date
    harvest_date = models.DateField(
        null=True, blank=True, help_text="When was this harvested or prepared?"
    )

    # Stock Alerts
    low_stock_threshold = models.PositiveIntegerField(
        default=5, help_text="Alert me when stock drops to or below this number."
    )
    low_stock_notified = models.BooleanField(
        default=False, help_text="Internal flag to prevent spamming emails."
    )

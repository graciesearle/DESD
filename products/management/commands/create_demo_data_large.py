...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            "\n  Run 'python manage.py create_demo_data_large' to populate."
        ))

    def _create_large_scale_orders(self, customer_map, product_map):
        self.stdout.write("  Generating high-volume order history (100+ for Robert) …")
        ...
        for i in range(120):
            ...
            order = Order.objects.create(
                ...
                shipping_address=robert.customerprofile.delivery_address,
                shipping_postcode=robert.customerprofile.postcode,
                total_price=Decimal("0.00"),
                created_at=current_date
            )
            
            # Mix of loyal items and 1-2 random items
            basket = random.sample(loyal_items, k=random.randint(2, 4))
            basket += random.sample(all_prods, k=random.randint(0, 2))
            
            order_total = Decimal("0.00")
            for prod in set(basket): # Deduplicate
            ...
            # Create a success payment for each
            Payment.objects.create(
                order=order,
                amount=order_total,
                status=Payment.Status.SUCCESS,
                stripe_payment_intent_id=f"pi_large_{i}",
                created_at=current_date
            )

        # 2. Bulk orders for other customers (50 orders each)
        for email, customer in customer_map.items():
            ...
            for j in range(50):
                ...
                o = Order.objects.create(
                    customer=customer,
                    status=Order.Status.DELIVERED,
                    shipping_address=customer.customerprofile.delivery_address,
                    shipping_postcode=customer.customerprofile.postcode,
                    total_price=Decimal("25.00"),
                    created_at=c_date
                )
                p = random.choice(all_prods)
                OrderItem.objects.create(order=o, product=p, quantity=2, unit_price=p.price, total_price=p.price*2, product_name=p.name)
                Payment.objects.create(order=o, amount=p.price*2, status=Payment.Status.SUCCESS, created_at=c_date)

        self.stdout.write(f"    Total Orders Generated: {Order.objects.count()}")
